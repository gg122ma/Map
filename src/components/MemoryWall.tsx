...
const EMOJIS = ['💛', '😂', '🥹', '❤️', '✨', '🎓', '🏆', '📖', '☕', '🌙', '🤝', '💌', '🎮', '🎤', '⚡', '🌟', '🍛', '🏸', '🥇'];
const YEARS = Array.from({ length: 10 }, (_, i) => String(2016 + i));
...
export default function MemoryWall({ building, onClose }: MemoryWallProps) {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    addMemory(building.id, { author: author.trim(), year, text: text.trim(), emoji });
    setSubmitted(true);
    setTimeout(() => {
      setSubmitted(false);
      setShowForm(false);
    ...
    >
      ...
      >
        ...
        <div
          ...
          style={{ background: `linear-gradient(135deg, ${building.color}33, ${building.roofColor}22)` }}
        >
          <div className="absolute top-0 left-0 right-0 h-1 rounded-t-full" style={{ background: building.color }} />

          <div className="flex items-start justify-between gap-4">
            <div className="flex items-center gap-3">
              <div
                className="w-14 h-14 rounded-2xl flex items-center justify-center text-3xl shadow-lg flex-shrink-0"
                style={{ background: `${building.color}44`, border: `2px solid ${building.color}88` }}
              >
                {building.emoji}
              </div>
              <div>
                <h2 className="text-2xl font-bold text-white leading-tight">{building.name}</h2>
          ...
          <p className="mt-3 text-sm text-slate-300 leading-relaxed italic">"{building.description}"</p>

          {/* Stats */}
          <div className="flex gap-3 mt-4">
            <div className="px-3 py-1.5 rounded-full bg-white/10 text-xs text-white font-medium">
              💬 {currentBuilding.memories.length} memories
            </div>
          ...
          <div className="px-6 py-3 flex gap-2 flex-wrap border-b border-white/5 flex-shrink-0">
            <button
              ...
              className={`px-3 py-1 rounded-full text-xs font-medium transition-all ${
                !activeFilter ? 'bg-white text-slate-900' : 'bg-white/10 text-slate-300 hover:bg-white/20'
              }`}
            >
              All
            </button>
            {uniqueYears.map(y => (
              <button
                ...
                className={`px-3 py-1 rounded-full text-xs font-medium transition-all ${
                  activeFilter === y ? 'text-white' : 'bg-white/10 text-slate-300 hover:bg-white/20'
                }`}
        ...
        <div ref={wallRef} className="flex-1 overflow-y-auto px-6 py-4 space-y-4 custom-scrollbar">
          <AnimatePresence mode="popLayout">
            {filteredMemories.length === 0 ? (
              ...
              >
                ...
                <p className="font-medium text-white">No memories yet for this filter</p>
                ...
                >
                  <div className="flex items-start gap-3">
                    ...
                    <div className="flex-1 min-w-0">
                      <p className="text-slate-100 text-sm leading-relaxed">{memory.text}</p>
                      <div className="flex items-center gap-2 mt-2">
                        <span
                          className="text-xs font-semibold px-2 py-0.5 rounded-full"
                          style={{ background: `${building.color}33`, color: building.color }}
                        >
                          {memory.author}
                        </span>
                        <span className="text-xs text-slate-500">Class of {memory.year}</span>
                      </div>
                    </div>
                  </div>
                </motion.div>
          ...
          <AnimatePresence mode="wait">
            {!showForm ? (
              <motion.button
                ...
                style={{ background: `linear-gradient(135deg, ${building.color}, ${building.roofColor})` }}
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
                exit={{ opacity: 0 }}
              ...
              >
                ...
                <p className="text-white font-semibold">Memory added!</p>
                <p className="text-slate-400 text-sm">Future students will find your story here.</p>
              </motion.div>
            ) : (
              <motion.form
                key="form"
              ...
              >
                ...
                <textarea
                  placeholder="Share your memory here... what made this place special to you?"
                  value={text}
                    ...
                    {EMOJIS.map(em => (
                      <button
                        ...
                        className={`w-8 h-8 rounded-lg text-base flex items-center justify-center transition-all ${
                          emoji === em ? 'scale-110 ring-2' : 'bg-white/5 hover:bg-white/15'
                        }`}
                        style={emoji === em ? { background: `${building.color}44`, outline: `2px solid ${building.color}` } : {}}
                      >
                        {em}
                      </button>
                    ))}
